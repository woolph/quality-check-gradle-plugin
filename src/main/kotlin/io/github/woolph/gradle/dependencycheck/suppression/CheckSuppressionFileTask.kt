...
/**
 * This task checks the suppression file provided for "inappropriate" suppression entries.
 * An entry is considered appropriate it
 * - it contains a FALSE POSTIVE "tag" in the notes, which indicates that the suppressed vulnerability is not affecting
 *   the application.
 * - or it has a expiration date which is not farther way than the maxSuppressionUntil (usually a year from now).
 *   Passing the expiration date disables the suppression entry, forcing you to reevaluate the vulnerability. Therefore
 *   this expiring suppression may be useful for vulnerabilities that, for some reason, cannot be fixed right away.
 */
    ...
    fun checkSuppressionFile() {
        originalSuppressionFile.asFile.orNull
            ...
            ?.let { originalSuppressionEntries ->
                ...
                val inappropriateEntries = originalSuppressionEntries
                    .filter {
