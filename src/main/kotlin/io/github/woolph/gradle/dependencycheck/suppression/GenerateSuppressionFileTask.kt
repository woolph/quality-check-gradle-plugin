...
abstract class GenerateSuppressionFileTask : DefaultTask() {
    ...
    init {
        ...
        suppressUntil.convention(
            project.providers
                .gradleProperty("suppressUntil")
                .map { SuppressionEntry.parseToZoneDateTime(it) }
    ...
    fun generateSuppressionFile() {
        ...
        val newSuppressionEntries =
            dependencyCheckXmlReport.asFile.get().processXml { doc ->
        ...
        val usedOriginalSuppressionEntries =
            if (includeAlreadySuppressed && originalSuppressionFile.isPresent) {
                val skippedVulnerabilities =
                    dependencyCheckXmlReport.asFile.get().processXml { doc ->
                        doc.children()
                            .flatMap { testsuites ->
                                testsuites
                                    .children()
                                    .filter { (it.attributes["skipped"]?.value?.toInt() ?: 0) > 0 }
                                    .children()
                                    .mapNotNull { testcase ->
                ...
                val originalSuppressionEntries =
                    originalSuppressionFile.asFile.get().parseAsDependencyCheckSuppressionFile()

                originalSuppressionEntries
                    .filter { it.vulnerabilities.any { it.name in skippedVulnerabilities } }
                    .mapNotNull {
                        val suppressedVulnerabilities =
                            it.vulnerabilities.filter { it.name in skippedVulnerabilities }
        ...
        sequenceOf(usedOriginalSuppressionEntries, newSuppressionEntries)
            .flatMap { it }
